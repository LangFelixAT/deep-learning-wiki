...
- For Gaussian corruption `x = y + n` with `n ~ N(0, sigma^2 I)`, the score and denoiser are related by:
...
- Needs verification: older VP/DDPM notation often uses cumulative products such as `alpha_bar_t`; mapping those to `sigma` depends on the chosen scaling convention.

## Relationship between x, noise, and network input
- The network usually does not have to operate on raw `x` directly.
- EDM writes the denoiser around a raw neural network `F_theta`:
...
- DDPM-style `epsilon_theta`, score prediction, and denoiser prediction can often be converted into one another when the noise schedule is known.
