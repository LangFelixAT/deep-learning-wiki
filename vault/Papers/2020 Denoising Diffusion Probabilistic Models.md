...
- Define a learned reverse process `p_theta(x_0:T)` that starts from Gaussian noise and samples backward toward data.
...
- Sampling starts from `x_T ~ N(0, I)` and repeatedly applies learned reverse transitions.
...
`L = E_q[-log p_theta(x_0:T) + log q(x_1:T|x_0)]`, with `E[-log p_theta(x_0)] <= L`.
